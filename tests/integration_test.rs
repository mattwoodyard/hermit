...
/// Check whether pasta is installed on this system.
fn has_pasta() -> bool {
    std::path::Path::new("/usr/bin/pasta").exists()
}

#[test]
fn test_net_pasta_basic_execution() {
    if !has_pasta() {
        eprintln!("skipping: pasta not installed");
        return;
    }
    let output = hermit_bin()
        .args(["--net", "pasta", "--project-dir", "/tmp", "--", "echo", "hello"])
        .output()
        .expect("failed to run hermit");
    assert!(
        output.status.success(),
        "hermit --net pasta failed: stderr={}",
        String::from_utf8_lossy(&output.stderr)
    );
    assert_eq!(String::from_utf8_lossy(&output.stdout).trim(), "hello");
}
...
#[test]
fn test_net_pasta_has_tap_interface() {
    if !has_pasta() {
        eprintln!("skipping: pasta not installed");
        return;
    }
    // Pasta creates a tap device in the child's net namespace, so there should
    // be more than just "lo" in /sys/class/net/
    let output = hermit_bin()
        .args([
            "--net", "pasta",
            "--project-dir", "/tmp",
    ...
    assert!(
        ifaces.len() > 1,
        "expected more than just lo with pasta, got: {:?}",
        ifaces
    );
}

#[test]
fn test_net_pasta_has_ip_address() {
    if !has_pasta() {
        eprintln!("skipping: pasta not installed");
        return;
    }
    // Pasta provides DHCP — the tap interface should have an IPv4 address.
    // We check by looking for an inet line on a non-lo interface.
    let output = hermit_bin()
        .args([
            "--net", "pasta",
            "--project-dir", "/tmp",
            "--",
            "sh", "-c",
            // ip addr show lists all interfaces; grep for inet (IPv4)
            // excluding loopback. Pasta typically assigns 192.168.* or 10.0.2.*
            "ip addr show 2>/dev/null || cat /proc/net/if_inet6 2>/dev/null && echo has_ip || echo no_ip_tool",
        ])
        .output()
        .expect("failed to run hermit");
    assert!(
    ...
    // Just verify we got some output — the IP tools may vary by system
    let stdout = String::from_utf8_lossy(&output.stdout);
    assert!(
        !stdout.trim().is_empty(),
        "expected some network info output"
    );
}

#[test]
fn test_net_pasta_exit_code_propagates() {
    if !has_pasta() {
        eprintln!("skipping: pasta not installed");
        return;
    }
...
#[test]
fn test_net_pasta_preserves_filesystem_isolation() {
    if !has_pasta() {
        eprintln!("skipping: pasta not installed");
        return;
    }
    // Verify that filesystem isolation still works with --net pasta:
    // workdir writes persist, /tmp writes are ephemeral
    let dir = tempfile::tempdir().expect("failed to create temp dir");
    let dir_path = dir.path().to_str().unwrap();
    let persist_file = format!("{}/pasta_persist_test", dir_path);

    let script = format!(
        "echo persisted > {} && echo ephemeral > /tmp/hermit_pasta_fs_test",
        persist_file
    );
    ...
    // Workdir write persisted
    let content = std::fs::read_to_string(&persist_file)
        .expect("workdir write did not persist with --net pasta");
    assert_eq!(content.trim(), "persisted");

    // /tmp write was ephemeral
    assert!(
        !std::path::Path::new("/tmp/hermit_pasta_fs_test").exists(),
        "/tmp write persisted with --net pasta"
    );
}

// --- SNI-proxy with pasta tests ---

fn has_sni_proxy() -> bool {
    which::which("sni-proxy").is_ok()
}

#[test]
fn test_net_pasta_with_proxy_starts() {
    if !has_pasta() || !has_sni_proxy() {
        eprintln!("skipping: pasta or sni-proxy not installed");
        return;
    }
    let output = hermit_bin()
        .args([
    ...
    assert!(
        output.status.success(),
        "hermit --net pasta --allowed-hosts failed: stderr={}",
        String::from_utf8_lossy(&output.stderr)
    );
    assert_eq!(String::from_utf8_lossy(&output.stdout).trim(), "proxy_ok");
}

#[test]
fn test_net_pasta_with_proxy_exit_code() {
    if !has_pasta() || !has_sni_proxy() {
        eprintln!("skipping: pasta or sni-proxy not installed");
        return;
    }
    let status = hermit_bin()
        .args([
            "--net", "pasta",
            "--allowed-hosts", "example.com,github.com",
            "--project-dir", "/tmp",
            "--",
            "sh", "-c", "exit 42",
        ])
