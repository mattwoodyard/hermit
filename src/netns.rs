...
pub(crate) fn set_interface_up(sock: i32, ifname: &str) -> Result<()> {
...
pub(crate) const TABLE_NAME: &str = "hermit_nat";
...
pub(crate) fn hermit_table() -> Table {
...
pub(crate) fn check_netlink_acks(buf: &[u8]) -> Result<()> {
...
pub(crate) fn output_chain(table: &Table) -> Chain {
    Chain::new(table)
        .with_name("output")
...
/// Wrappers around `netns`'s private items for the dedicated test
/// crate. Off by default; `hermit-tests` flips on the
...
pub mod __test_internals {
    use anyhow::Result;
    use rustables::{Chain, Table};

    pub const TABLE_NAME: &str = super::TABLE_NAME;

    pub fn hermit_table() -> Table {
        super::hermit_table()
    }

    pub fn output_chain(table: &Table) -> Chain {
        super::output_chain(table)
    }

    pub fn set_interface_up(sock: i32, ifname: &str) -> Result<()> {
        super::set_interface_up(sock, ifname)
    }
